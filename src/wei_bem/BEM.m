...
BS=importdata('Blade/blade_section/blade_section.dat').data;

%import Aero data files
Readfiles = dir(fullfile('Blade/aero_data','*.dat'));
for i=1:length(Readfiles)
    AD{i}=importdata(strcat('Blade/aero_data/',Readfiles(i).name));
end
