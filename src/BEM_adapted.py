...
def bem(v0, omega, pitch_deg):
    ...
    n_sections = len(blade_section)         # Number of blade sections radii = np.zeros(n_sections)            # Blade section locations
    radii = np.zeros(n_sections)
...
def bem_fsi_new(v0, v_blade_ip, v_blade_oop, omega, pitch_deg):
    # Fixed parameters
    B = 3             # Number of blades
    R = 63            # Rotor radius
    hub_rad = 1.5     # Hub radius
    ...
    for i in range(n_sections):
        ...
        #f_normal[i] = 0.5 * rho * ((r * omega * (1 + a_prime)) ** 2 + (v0 * (1 - a)) ** 2) * chord * cn * dr
        #f_tangential[i] = 0.5 * rho * ((r * omega * (1 + a_prime)) ** 2 + (v0 * (1 - a)) ** 2) * chord * ct * dr
        f_normal[i] = 0.5 * rho * ((r * omega * (1 + a_prime) - v_blade_ip[i]) ** 2 + (v0 * (1 - a)- v_blade_oop[i]) ** 2) * chord * cn * dr
        f_tangential[i] = 0.5 * rho * ((r * omega * (1 + a_prime)- v_blade_ip[i]) ** 2 + (v0 * (1 - a)- v_blade_oop[i]) ** 2) * chord * ct *dr
    ...
    for i in range(n_sections):
        ...
        while abs(ax - a) >= precision_tolerance or abs(ax_prime - a_prime) >= precision_tolerance:
            ...
            if False:
                ...
                print(f"f is:{f_tiploss}")
                print(f"fhub is:{f_hubloss_pre}")
                print(f"fhub is:{f_hubloss}")
            f = f_tiploss * f_hubloss
            #if f>1:
            #    f=1
            #if f<0:
            ...
            if ax > ac:
                ...
                ax = 1 / (4 * f * np.sin(np.deg2rad(phi)) ** 2 / (sigma * cn) + 1)
            #ax = 0.5* ax_old + 0.5*ax  # add some underrelaxation
        ...
        cl_list[i] = cl_current
        cd_list[i] = cd_current
    M_t = np.sum(f_tangential * dr_list * radii)        # Rotor torque from one blade
    M_n = np.sum(f_normal * dr_list * radii)        # Rotor torque from one blade
    power = M_t * omega * 3 * 0.944           # Power
    return radii, f_normal, f_tangential, induction_axial, induction_tangential, M_t, M_n, power
